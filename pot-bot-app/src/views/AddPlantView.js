...
export default function AddPlantView({ addPlantToPersonalList }) {
  ...
  const handleLoadMore = async () => {
    ...
    const plantDetails = await Promise.all(validIDs.map((plant) => fetchPlantDetails(plant.id)));
  ...
  return (
    <div className="addPlant">
      ...
      <form className="plant-form" onSubmit={handleSubmit}>
        <input type="text" placeholder="Choose your plant" value={searchTerm} onChange={handleChange} />
      ...
      <div className="search-results-grid">
        {searchResults.map((plant, index) => (
          <div className="plant-card" key={plant.id} onClick={() => handlePlantClick(plant.id)}>
            <div key={plant.id}>
              {plant.default_image && (
            ...
            {expandedPlantId === plant.id && (
              <div className="plant-dropdown">
                <button className="add-plant-button" onClick={() => handleAddPlantButtonClick(plant)}>
                  Add to my plants
                </button>
              </div>
            )}
