 ...
 * Maybe a listener for the water value, think that onValue could do this. Need to read
 * about it.
 * @param {*} user
 */
function setWateredTrue(user, name) {
  const path = `plants/${name}/settings`;
  const data = {water: 1};
  console.log("watered plant");

  return updatePlantData(user, path, data);
  /**TODO
